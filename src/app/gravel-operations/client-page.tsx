  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex justify-between">
        <h1 className="text-3xl font-bold">Cascalheira</h1>
        <button onClick={() => setShowModal(true)} className="bg-primary text-primary-foreground px-4 py-2 rounded">Nova Extração</button>
      </div>
        ...
        <tbody>
          {records.map((r: any) => (
            <tr key={r.id} className="border-t">
              ...
              <td className="p-4">{r.volume_extracted}</td>
              <td className="p-4">
                <button onClick={() => deleteGravel(r.id)} className="text-red-500">Excluir</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-background p-6 rounded max-w-md w-full">
            <h3 className="text-xl mb-4">Registrar Extração</h3>
            <form action={async (fd) => { await createGravel(fd); setShowModal(false); }} className="space-y-4 flex flex-col">
              ...
              <input name="volume" type="number" step="0.01" placeholder="Volume (m³)" required className="border p-2" />
